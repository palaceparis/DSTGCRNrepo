...
def objective_function(tuned_args: DictConfig, args: DictConfig) -> float:
    if not args.hyperopt_tuning:
        ...
        try:
            ...
            args.embed_dim = int(tuned_args["embed_dim"])
            args.num_layers = int(tuned_args["num_layers"])
            args.rnn_units = int(tuned_args["rnn_units"])
            # args.cheb_k = int(tuned_args["cheb_k"])
            args.lr_init = tuned_args["lr_init"]
            # args.num_heads = int(tuned_args["num_heads"])
            args.hidden_dim_node = int(tuned_args["hidden_dim_node"])
            args.num_layers_node = int(tuned_args["num_layers_node"])
            # args.input_dim = int(tuned_args["input_dim"])
